...
{#snippet navIcon(icon: string)}
	{#if icon === 'folder-plus'}
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" class="w-5 h-5 shrink-0">
...
<div class="min-h-screen bg-tw-darkblue flex">
	<!-- Sidebar -->
	<aside
...
<Modal bind:open={showNewFolder} title="New Folder">
	<form onsubmit={(e) => { e.preventDefault(); createFolder(); }} class="flex flex-col gap-4">
		...
		<button
			...
			class="rounded-lg py-2.5 font-semibold text-white
			       ...
			       focus:outline-none focus:ring-2 focus:ring-tw-neon
			       {folderName.trim() ? 'bg-tw-purple hover:bg-tw-pink cursor-pointer' : 'bg-white/10 text-white/30 cursor-not-allowed'}"
		>
			Create
		</button>
	</form>
